...
let sub = ''; 
let subconverter = 'subapi-loadbalancing.pages.dev';// clash订阅转换后端，目前使用AM的订阅转换功能。自带虚假节点信息防泄露
...
`;
		...
		let url = `https://${sub}/sub?host=${fakeHostName}&pw=${fakeUserID}&password=${fakeUserID}&epeius=am-trojan&proxyip=${RproxyIP}`;
		let isBase64 = true;
		let newAddressesapi = [];
		let newAddressescsv = [];
		...
		try {
			...
			if ((!sub || sub == "") && isBase64 == true) {
				...
				const response = await fetch(url ,{
					headers: {
						'User-Agent': `am-trojan/ansoncloud8`
					}});
